...
class SelfOrganizingMap:
    ...
    def _init_weights(self) -> np.ndarray:
        return np.random.random(
            (self.config.width, self.config.height, self.config.input_dim)
        )
    ...
    def _find_bmu(self, vector: np.ndarray) -> Tuple[int, int]:
        ...
        bmu_x, bmu_y = np.unravel_index(
            np.argmin(distances), (self.config.width, self.config.height)
        )
        return bmu_x, bmu_y
    ...
    def _update_weights(
        self, vector: np.ndarray, bmu_x: int, bmu_y: int, alpha: float, sigma: float
    ):
        """
